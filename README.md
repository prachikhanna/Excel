...
*please note: screenshots are provided as a helpful example of what your result MAY look like, but have been produced from different variations of the dataset!*
...
* The dates stored within the `deadline` and `launched_at` columns use Unix timestamps. Fortunately for us, [there is a formula](http://spreadsheetpage.com/index.php/tip/converting_unix_timestamps/) that can be used to convert these timestamps to a normal date.
  ...
  * Create a new sheet with a pivot table with a column of `state`, rows of `Date Created Conversion`, values based on the count of `state`, and filters based on `parent category` and `years`. You will need to calculate years from the 'epoch times' provided in the deadline/launched_at columns. You can use the launched_at time's year as the date, but note that this is a simplification, given that projects can span multiple years. 
